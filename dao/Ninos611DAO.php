...
require_once ($_SERVER['DOCUMENT_ROOT'].'/'.$projectName.'/model/Ninos611.php');

class Ninos611DAO extends Database {
    ...
    function saveNinos611($ninos){
        
        $this->connect();
        
       
        $queryNino = "INSERT INTO  ninos611 (tipoide, numero,fecha_nac,sexo,cod_etnia,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,fecha_atencion,finalidad_consulta,codigo_cups,peso,talla,fecha_hemoglobina,resulado_hemoglobina    ) VALUES (
    ...
    function updateNinos611($ninos){
        
        $this->connect();
        //echo "etnia ".$recien->etnia. " etnia";    
        $queryNino = "UPDATE  ninos611  SET    tipoide = '$ninos->tipo_id', 
    ...
    function getInfoNinos611($id){
        ...
        $query = "SELECT * FROM detalle_6_11 where id = $id";
        $this->connect();
        $info = $this->ejecutar($query);

        $persona = new Ninos611;
